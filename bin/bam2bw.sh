...
do
	...
	if [ "$expType" == "rnaseq" ]; then
		cmd="$cmd  bamCoverage -b $bam -o $b.fw.bw  --minMappingQuality 1 -bs 2 --normalizeUsingRPKM -p $procs --filterRNAstrand forward" 
		echo "$cmd" >> $commandFile
		cmd=""
		cmd="$cmd  bamCoverage -b $bam -o $b.rev.bw  --minMappingQuality 1 -bs 2 --normalizeUsingRPKM -p $procs --filterRNAstrand reverse" 
        echo "$cmd" >> $commandFile
    else
        cmd="$cmd bamCoverage -b $bam -o $b.bw  --minMappingQuality 1 -bs 2 --normalizeUsingRPKM -p $procs" 
        echo "$cmd" >> $commandFile
    fi
done < $inputFile
